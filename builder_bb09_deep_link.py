...
wf = {
    ...
    "nodes": [
        { 
            "parameters": { "httpMethod": "GET", "path": "agendar/:slug", "responseMode": "responseNode" }, 
            "id": "web_deep", "name": "GET /agendar/:slug", "type": "n8n-nodes-base.webhook", "typeVersion": 1, "position": [0, 0] 
        },
        ...
        { 
            "parameters": { 
                ...
                "query": "SELECT slug FROM public.professionals WHERE slug = $1 AND deleted_at IS NULL",
                "options": { "queryParameters": { "values": [ { "value": "={{ $json.params.slug }}" } ] } }
            }, 
            "id": "db_lookup", "name": "DB: Find Doctor", "type": "n8n-nodes-base.postgres", "typeVersion": 2.4, "position": [250, 0], "credentials": {"postgres": {"name": "Postgres Neon"}} 
        },
        ...
        # REDIRECT (FOUND)
        # Note: Replace 'TuBot' with variable later
        { 
            "parameters": { 
                "respondWith": "redirect", 
                "redirectUrl": "={{ 'https://t.me/' + $env.TELEGRAM_BOT_NAME + '?start=ref_' + $json.slug }}" 
            }, 

...
class N8NCrudAgent:
    ...
    def execute_workflow(self, workflow_id: str, data: Optional[Dict] = None) -> Optional[Dict]:
        ...
        Args:
            ...
            data: Optional data payload to pass to the workflow

        Returns:
            Execution result or None if error occurs
        """
        try:
            url = f"{self.api_url}/api/v1/workflows/{workflow_id}/run"
            ...
            if response.status_code == 200:
                ...
                print(f"Error executing workflow {workflow_id}: {response.status_code} - {response.text}")
                return None
        except Exception as e:
            print(f"Error connecting to n8n: {str(e)}")
            return None

    def get_executions(self, workflow_id: str = None, limit: int = 10) -> Optional[List[Dict]]:
        """
        Get executions for a specific workflow or all workflows

        Args:
            workflow_id: ID of the workflow (optional, gets all if not provided)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Error retrieving executions: {response.status_code} - {response.text}")
                return None
        except Exception as e:
            print(f"Error connecting to n8n: {str(e)}")
            return None
    ...
    def get_execution_by_id(self, execution_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Error retrieving execution {execution_id}: {response.status_code} - {response.text}")
                return None
        except Exception as e:
            print(f"Error connecting to n8n: {str(e)}")
            return None
...
def main():
    ...
    print("9. Execute a workflow")
    print("10. Get executions")

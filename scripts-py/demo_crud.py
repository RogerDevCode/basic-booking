...
def demo_crud_operations():
    ...
    # 7. EXECUTE: Execute the workflow manually
    print(f"\n--- 7. EXECUTE Operation ---")
    execution_result = agent.execute_workflow(workflow_id)
    if execution_result:
        print(f"✓ Successfully executed workflow: {workflow_name}")
        print(f"  Execution ID: {execution_result.get('id')}")
    else:
        print(f"✗ Failed to execute workflow: {workflow_name}")

    # 8. GET EXECUTIONS: Get recent executions
    print(f"\n--- 8. GET EXECUTIONS Operation ---")
    executions = agent.get_executions(workflow_id, limit=5)
    if executions:
        print(f"Found {len(executions)} recent executions:")
        for exec_data in executions:
            print(f"  - ID: {exec_data.get('id')} | Status: {exec_data.get('status')}")
    else:
        print("No executions found.")

    # 9. DELETE: Clean up by deleting the workflow
    print(f"\n--- 9. DELETE Operation ---")
...
def show_available_methods():
    ...
    print("• execute_workflow(id) - Execute workflow manually")
    print("• get_executions(workflow_id, limit) - Get workflow executions")
    print("• get_execution_by_id(id) - Get specific execution")

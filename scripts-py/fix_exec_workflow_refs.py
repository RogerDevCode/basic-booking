#!/usr/bin/env python3
"""
Fix Execute Workflow nodes to use workflow IDs instead of names.
"""
...
def fix_workflow(filepath):
    ...
    for node in nodes:
        if node.get('type') == 'n8n-nodes-base.executeWorkflow':
            ...
            # Check if it's using name instead of ID
            if wf_ref.get('mode') == 'list' and wf_ref.get('cachedResultName'):
                old_name = wf_ref['cachedResultName']
                if old_name in WORKFLOW_IDS:
                    # Update to use ID
                    wf_ref['value'] = WORKFLOW_IDS[old_name]
                    wf_ref['mode'] = 'id'
                    if 'cachedResultName' in wf_ref:
                        del wf_ref['cachedResultName']
                    fixed = True
                    print(f"  [FIX] {workflow_name}: {node.get('name')} - Changed '{old_name}' to ID {WORKFLOW_IDS[old_name]}")
...
def main():
    print("Fixing Execute Workflow references to use IDs...")
    print("=" * 60)

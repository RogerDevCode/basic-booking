#!/usr/bin/env python3
"""
Fix Execute Workflow nodes - use 'list' mode with both cachedResultName and value (ID).
"""
...
def fix_workflow(filepath):
    ...
    for node in nodes:
        if node.get('type') == 'n8n-nodes-base.executeWorkflow':
            ...
            # Get the workflow name from the current setup
            wf_name = wf_ref.get('cachedResultName', '') or wf_ref.get('value', '')
            
            # If it's an ID, find the name
            if wf_name in WORKFLOW_IDS.values():
            ...
            if wf_name in WORKFLOW_IDS:
                # Set proper format: mode=list, value=ID, cachedResultName=name
                wf_ref['mode'] = 'list'
                wf_ref['value'] = WORKFLOW_IDS[wf_name]
                wf_ref['cachedResultName'] = wf_name
                wf_ref['__rl'] = True
                fixed = True
                print(f"  [FIX] {workflow_name}: {node.get('name')} -> {wf_name} (ID: {WORKFLOW_IDS[wf_name]})")
...
def main():
    print("Fixing Execute Workflow references...")
    print("=" * 60)
